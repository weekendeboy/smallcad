...
export const CADSketchCanvas: React.FC = () => {
  ...
  const { activeSketchId, document, selectedEntityIds } = useCADStore();
  ...
  const handlePointerMove = useCallback(
    ...
    [viewportHandlers, screenToWorld]
  );
  ...
  useEffect(() => {
    if (dimensions.width > 0 && dimensions.height > 0 && pan.x === 0 && pan.y === 0) {
      // 這裡無法直接呼叫 setPan，為了簡單起見，我們假設初始 pan 會被設在 {width/2, height/2} 會更好。
      // 但用 hook 時 initialPan 已經是 {0, 0}
